...
const pressButtonFaqES = [
  {
    q: "¿Cuál es la diferencia entre un skill de IA y una tarea automática?",
    a: "Un skill es un conjunto de instrucciones guardadas que produce el mismo tipo de resultado cada vez que lo ejecutas. Una tarea automática es ese mismo skill, pero lo arranca algo que no eres tú: un horario, un formulario enviado, una llamada que termina, un archivo que aparece en una carpeta. El skill no cambia. Lo que cambia es que el negocio sigue avanzando cuando no estás frente a la computadora.",
  },
  {
    q: "¿Qué papel tiene la persona si la IA trabaja sola?",
    a: "La persona deja de arrancar el trabajo y pasa a aprobarlo. La tarea se ejecuta, el resultado llega a un lugar que ya revisas y tú lo apruebas, lo corriges o lo descartas. Todo lo que sale del negocio, como un correo a un cliente, un precio o una página publicada, sigue esperando esa aprobación antes de salir.",
  },
  {
    q: "¿Qué puede arrancar un flujo de IA de forma automática?",
    a: "Dos tipos de cosas. Un horario, como todos los días a las 7 de la mañana o cada lunes, que en la jerga se llama cron. O un evento, como un formulario enviado, una llamada que termina, un correo que llega o un archivo nuevo, que normalmente le llega a la IA por medio de un webhook. Casi todos los negocios pequeños ya funcionan con los dos tipos de ritmo; el trabajo está en conectarlos con un skill.",
  },
  {
    q: "¿Qué tareas de un negocio no conviene automatizar por completo con IA?",
...
const agentStackFaqEN = [
  {
    ...
    a: "A chatbot waits for you to open it and forgets everything when you close the tab. An agent has a named job, a trigger that is not you, context that survives between runs, tools it can reach, and a place the work lands. The model can be the same in both cases. The five pieces around it are the difference.",
  },
  {
    q: "What is an agent loop, and what is an agent graph?",
    a: "A loop is one agent doing the same job again on a trigger you set once. A graph is several agents handing work to each other, one drafting and another checking. For most small businesses the return starts at the loop, and a loop does not require code.",
  },
  {
    q: "Is GrokBot or Claude better for a small business?",
    a: "They are better at different halves. GrokBot is the easier start and the better home for always-on work: routing, routines, browser logins, follow-through with nobody at a keyboard. Claude in a code editor is better for anything touching files, repositories or deploys. If you will only run one thing this year, start with the accessible one.",
  },
  {
    q: "Do I need to know how to code to build an AI agent?",
    a: "Not for your first one. The hard part is describing the job clearly enough that something else can follow it, and that is writing, not programming. Coding matters when you need an agent to change something inside a system it cannot reach from a browser, and most businesses take months to hit that wall.",
  },
  {
    q: "Is it worth running two AI platforms instead of one?",
    a: "It is worth it when the second one can genuinely disagree with the first. Two agents on one platform share the same blind spots, so they mostly agree, which feels like verification but is not. The cost is two memories to feed and a decision about which half owns each job. If you will not maintain both, run one.",
  },
  {
...
export const posts: Post[] = [
    ...
    content: (
      <>
        ...
        <p>
          ...
          users settle into and mistake for the top. Turning prompts into
          skills is real progress, and we argued for it in{" "}
        ...
        <p>
          Only what starts it. The instructions, the context and the output
          format all carry over. At the button, you start it. Past the button,
          something in the business does: a time of day, a form submission, a
          call ending, a file landing in a folder. You need to answer one
          question per skill: what event in my business should mean this work
          starts?
        </p>
        <p>
          The trigger is one of the five pieces we walked through in{" "}
        ...
        <p>
          You stop doing the task and start judging it: approve it, fix it, or
          throw it out. That only works if the output is built to be judged
          fast. Our site watcher runs at seven every morning and nobody starts
          it. It checks twenty things across the sites we are responsible for.
          When everything is fine, it says so in one line.
        </p>
        <p>
          That one line is the most important design decision in the setup. If
          the healthy report ran a page long, I would stop reading it by the
          second week. An automated job whose output you have stopped reading
          is worse than a manual one, because it looks like coverage.
        </p>

        <h2>Why did our check pass by hand and fail on schedule?</h2>
        <p>
          This month we added a check that compares the listings on a real
          estate client&apos;s site against the MLS. We ran it by hand twice
          and it passed. Then it failed on its first scheduled run, and on the
          one after that.
        </p>
        <p>
          Nothing on the client&apos;s site was wrong. On a Mac, the scheduler
          is not allowed to read files on the Desktop, and the check kept its
          reference copy there. When I ran it, it had my permissions. When the
          schedule ran it, it didn&apos;t.
        </p>
        <p>
          Pressing the button tests your own context: your logins, your files,
          your permissions. The trigger runs in a different one. A skill is not
          automated until it has worked in the context the trigger uses.
        </p>

        <h2>Which skills should go on a trigger first?</h2>
        <p>
          The ones you have run by hand often enough to be bored. A skill is
          judgment written down (we made that case in{" "}
          <a href="/blog/will-ai-take-my-job-build-the-machine">
            the people who build the machine
          </a>
          ), and one you still tweak on every run isn&apos;t finished. Good
          candidates:
        </p>
        <ul>
          <li>
            Follow a rhythm the business already has: every morning, every new
            lead, every finished call.
          </li>
          <li>Get their input somewhere a machine can see it.</li>
          <li>Land with you before they reach anyone else.</li>
          <li>Can be judged good or bad in under a minute.</li>
        </ul>
        <p>
          One we haven&apos;t moved yet: every call we record is written to
          disk as a transcript when it ends, and I still ask for the notes by
          hand. It is the obvious next one off the button.
        </p>

        <h2>Which jobs should never run on a trigger?</h2>
        <p>
          My rule is that nothing leaves the building on a trigger alone. A
          trigger can prepare a client email, a price change or a reply to a
          review, and a person approves it before anyone outside sees it.
          People usually get this backwards: they automate the send because it
          is the tedious click. The preparation is what a trigger should do
          while you sleep. The click that puts your name on something is the
          piece worth keeping.
        </p>
        ...
        <p>
          ...
          check when you review its output; that list is your approval step.
          Send the output somewhere you already look every day. Name the event
          that should start it, then make that trigger run it once while you
          watch, not you pressing the button.
        </p>
        <p>
          Every morning at seven, twenty checks run across our sites, and most
          days the whole report is one line. I don&apos;t start it. Reading
          that line is the job now.
        </p>
  ...
  {
    slug: "tu-ia-sigue-esperando-que-aprietes-el-boton",
    faq: pressButtonFaqES,
    title: "Tu IA sigue esperando que aprietes el botón",
    excerpt:
      "Una biblioteca de skills parece la meta. Si cada ejecución todavía empieza contigo, el cuello de botella eres tú.",
    date: "19 de septiembre de 2026",
    dateISO: "2026-09-19",
    category: "Estrategia de IA",
    readTime: "7 min de lectura",
    coverImage: "/work/press-the-button-es.jpg",
    coverAlt:
      "Tarjeta oscura que dice Tu IA sigue esperando que aprietes el botón, sobre tres tarjetas: el chat, donde tú lo arrancas y haces el trabajo; el skill, donde tú lo arrancas y él hace el trabajo; y la tarea automática, donde otra cosa lo arranca y tú lo apruebas",
    keywords:
      "automatizar flujos de IA, skills de IA vs tareas automáticas, tareas de IA programadas, automatización con webhook, aprobación humana en IA, qué tareas automatizar con IA, automatización con IA para negocios pequeños, implementación de IA bilingüe Colorado",
    content: (
      <>
        <p>
          Una prueba para quien ya se volvió bueno usando IA: cuenta cuántas
          ejecuciones arrancaste esta semana. Cada resumen, cada borrador, cada
          reporte que produjeron tus skills guardados. ¿Quién apretó el botón?
        </p>
        <p>
          Si la respuesta es tú, todas las veces, estás en el nivel donde se
          quedan casi todos los usuarios serios, convencidos de que ya llegaron
          arriba. Escribiste tus instrucciones, las convertiste en skills
          (flujos de trabajo guardados que la IA sigue igual cada vez) y ahora
          el resultado es consistente. Es un avance real, y en abril lo
          defendimos en{" "}
          <a href="/blog/ai-learning-curve">la curva de aprendizaje de la IA</a>{" "}
          y en{" "}
          <a href="/blog/claude-code-vs-cowork">
            usas Claude, pero no lo estás entrenando
          </a>{" "}
          (ambos en inglés). El problema es que el negocio solo se mueve cuando
          tú te sientas a arrancarlo. Construiste una máquina rápida con una
          sola llave de encendido, y esa llave la llevas en el bolsillo.
        </p>

        <h2>¿Qué cambia cuando un skill corre sin ti?</h2>
        <p>
          Menos de lo que uno espera, y esa es la buena noticia. El skill sigue
          siendo el mismo: las instrucciones, el contexto y el formato del
          resultado se mantienen. Lo único que cambia es qué lo arranca.
        </p>
        <p>
          Con el botón, lo arrancas tú. Sin el botón, lo arranca algo que pasa
          en el negocio: una hora del día, un formulario enviado, una llamada
          que termina, un archivo que aparece en una carpeta. En la jerga, al
          horario le dicen cron y al evento, webhook, pero no necesitas ninguna
          de las dos palabras para usarlos. Lo que sí necesitas es responder una
          pregunta por cada skill: ¿qué tiene que pasar en mi negocio para que
          este trabajo empiece?
        </p>
        <p>
          Las cinco piezas que necesita un agente ya las explicamos en{" "}
          <a href="/blog/grokbot-o-claude-arquitectura-de-agentes">
            ¿GrokBot o Claude?
          </a>
          , así que no voy a repetir la lista. Este artículo trata de la pieza
          que casi todos se saltan, el disparador, y de lo que pasa con tu
          trabajo cuando el disparador ya no eres tú.
        </p>

        <img
          src="/work/press-the-button-flow-es.jpg"
          alt="Cuatro tarjetas que muestran un skill que corre sin que nadie lo arranque: un disparador a las 7 de la mañana, el skill haciendo veinte revisiones, un resultado de una línea que llega donde ya miras, y una persona que lo aprueba, lo corrige o lo descarta"
        />

        <h2>¿Qué significa que la persona solo aprueba?</h2>
        <p>
          Aprobar es revisar un trabajo que ya está hecho. Dejas de hacer la
          tarea y empiezas a juzgarla: la apruebas, la corriges o la descartas.
          Suena a que te bajaron de puesto. En la práctica, es la primera vez
          que tu criterio se usa en cada ejecución, en lugar de gastarse en
          acordarte de arrancarla.
        </p>
        <p>
          Eso solo funciona si el resultado está pensado para revisarse rápido.
          Nuestro monitor de sitios corre todos los días a las 7 de la mañana
          sin que nadie lo arranque. Hace veinte revisiones en los sitios de los
          que somos responsables, los nuestros y los de nuestros clientes.
          Cuando todo está bien, lo dice en una línea. Cuando algo falla, dice
          en qué sitio y cuál fue la revisión que falló.
        </p>
        <p>
          Esa única línea es la decisión de diseño más importante de todo el
          sistema. Si el reporte de un día normal ocupara una página, yo dejaría
          de leerlo en la segunda semana y, a partir de ahí, nadie estaría
          aprobando nada. Una tarea automática cuyo resultado ya nadie lee es
          peor que una manual, porque da la impresión de que alguien está
          vigilando.
        </p>

        <h2>¿Por qué nuestra revisión funcionaba a mano y fallaba con el horario?</h2>
        <p>
          El mismo monitor nos dejó la lección que yo pondría primero en
          cualquier lista sobre automatización. Este mes agregamos una revisión
          que compara los inmuebles publicados en el sitio de un cliente de
          bienes raíces con el MLS. La corrimos a mano y pasó. La corrimos otra
          vez y pasó. Después falló en su primera ejecución programada, y
          también en la siguiente.
        </p>
        <p>
          En el sitio del cliente no había nada mal: los siete inmuebles
          coincidían las dos mañanas. Lo que no podía correr era la revisión.
          En una Mac, el programador de tareas no tiene permiso para leer
          archivos del Escritorio, y la revisión guardaba su copia de referencia
          en una carpeta del Escritorio. Cuando la corría yo, corría con mis
          permisos. Cuando la corría el horario, no los tenía.
        </p>
        <p>
          Ese es el problema de probar una automatización apretando el botón:
          estás probando tu propio contexto, con tus sesiones abiertas, tus
          archivos y tus permisos. El disparador trabaja en otro contexto. Un
          skill no está automatizado hasta que funcionó en el contexto que usa
          el disparador, y por eso ahora, antes de dar por terminada una
          revisión nueva, obligamos al propio programador a ejecutarla.
        </p>
        <p>
          Cambiamos la revisión para que lea el sitio en vivo en lugar de un
          archivo, y resultó ser una mejor pregunta: ¿la página que ve el
          público coincide con el MLS? La segunda corrección fue más pequeña y
          pesó lo mismo. La ejecución fallida se había reportado como una lista
          de ocho problemas, porque el texto del error parecía una lista de
          problemas. Ahora, cuando una revisión no puede correr, lo dice en una
          línea y no saca ninguna conclusión sobre los inmuebles. Que una tarea
          no haya podido correr y que haya encontrado algo mal son dos hechos
          distintos, y la línea de la mañana tiene que mantenerlos separados.
        </p>

        <h2>¿Qué skills conviene poner primero en un disparador?</h2>
        <p>
          Los que ya corriste a mano tantas veces que te aburren. El
          aburrimiento indica que la información de entrada siempre llega igual
          y que ya sabes cómo se ve un buen resultado. Un skill es criterio
          puesto por escrito (lo explicamos en{" "}
          <a href="/blog/la-ia-me-va-a-quitar-el-trabajo">
            a los que construyen la máquina no los reemplaza la máquina
          </a>
          ), y un skill que todavía ajustas en cada ejecución es criterio que no
          terminaste de escribir. Los buenos candidatos tienen varias cosas en
          común:
        </p>
        <ul>
          <li>
            Siguen un ritmo que el negocio ya tiene: cada mañana, cada cliente
            nuevo, cada llamada que termina.
          </li>
          <li>
            La información llega a un lugar donde una máquina la puede ver,
            como una bandeja de entrada, una carpeta o un formulario.
          </li>
          <li>Un resultado equivocado te llega a ti antes que a nadie más.</li>
          <li>Puedes distinguir un buen resultado de uno malo en menos de un minuto.</li>
        </ul>
        <p>
          Hay uno que todavía no pasamos al disparador. Cada llamada que
          grabamos ya queda guardada como transcripción cuando termina.
          Convertir esa transcripción en notas y pendientes es un skill que uso
          todo el tiempo, y aun así lo sigo arrancando yo, pidiéndolo. El evento
          ya existe, la transcripción siempre tiene la misma forma y un mal
          resumen me cuesta un minuto. Es el siguiente candidato obvio para
          dejar de depender del botón.
        </p>

        <h2>¿Qué trabajos nunca deberían correr con un disparador?</h2>
        <p>
          Mi regla es que nada sale del negocio solo porque se activó un
          disparador. El disparador puede preparar un correo para un cliente,
          pero lo envía una persona. Puede redactar un cambio de precio, un
          artículo o la respuesta a una reseña, pero una persona lo aprueba
          antes de que alguien de afuera lo vea. Todo lo que no tiene vuelta
          atrás, como pagos, borrados o envíos, se queda detrás de una persona,
          aunque la redacción sea automática.
        </p>
        <p>
          Aquí es donde casi todos lo hacen al revés. Automatizan el final,
          porque enviar es el clic tedioso, y siguen preparando todo a mano
          porque sienten que ese es el trabajo de verdad. Hay que darle la
          vuelta. La preparación es justamente lo que un disparador debería
          hacer mientras duermes. El clic que pone tu nombre en algo es la parte
          que vale la pena conservar.
        </p>
        <p>
          Hay otro tipo de trabajo que conviene dejar fuera del disparador:
          cualquiera en el que la decisión cambia cada vez. Si no puedes poner
          por escrito cómo se ve un buen resultado, ese trabajo todavía no es un
          skill, y automatizarlo solo produce trabajo malo con horario fijo.
        </p>

        <h2>¿Cómo sacas tu primer skill del botón?</h2>
        <p>
          Elige el skill que más ejecutaste el mes pasado. Anota qué revisas
          cuando miras su resultado: esa lista se convierte en tu paso de
          aprobación, y casi siempre es más corta de lo que esperas. Decide
          dónde va a llegar el resultado, en algún lugar que ya revisas todos
          los días y no en un tablero nuevo que vas a olvidar. Después define
          qué evento lo tiene que arrancar y conecta las dos cosas.
        </p>
        <p>
          Antes de confiar en él, haz que el disparador lo ejecute una vez
          mientras miras. No tú apretando el botón, sino el horario o el evento
          real. Nuestra revisión pasó cada vez que la arranqué yo y falló cada
          vez que la arrancó el horario, y solo viendo una ejecución programada
          podía darme cuenta.
        </p>
        <p>
        ...
        <p>
          Todos los días a las 7 de la mañana corren veinte revisiones en los
          sitios de los que somos responsables, y casi siempre el reporte
          completo es una sola línea. No lo arranco yo y no tengo que acordarme
          de que existe. Mi trabajo ahora es leer esa línea.
        </p>

        <FAQ items={pressButtonFaqES} title="Preguntas frecuentes" />
    ...
    content: (
      <>
        <p>
          ...
          typing into a chat window for a year, they have seen a demo of an
          agent working with nobody in the room, and they want to know which
          one to buy.
        </p>
        <p>
          ...
          decision here. What separates a chatbot from an agent is five things
          you put around it, and those five are identical whoever built it.
        </p>
        ...
        <p>
          An agent has a job it does whether you show up or not. That takes
          five pieces, and none of them are the model. A job: one named
          responsibility, not &ldquo;help me with stuff.&rdquo; Context that
          survives the window closing, so it knows your clients, your prices,
          and the mistake you made in March that you refuse to repeat. Tools it
          can actually reach, which is the difference between telling you the
          page is broken and fixing the page. A trigger that is not you: a
          schedule, an inbound call, a file landing in a folder. And somewhere
          the work lands where a person will find it.
        </p>
        <p>
          Take any one away and you are back to a chat window with a better
          name. Put all five around an ordinary model and you have something
          that runs your Tuesday for you.
        </p>

        <h2>How far past a chatbot does this go?</h2>
        <p>
          A version making the rounds stacks it in five steps: chatbot, coding
          assistant, loop, graph, teammate. Most business owners I talk to are
          standing on the first rung and shopping for the fifth.
        </p>
        ...
        <p>
          ...
          that you stop watching every step and start reviewing results. The
          return starts at rung three, reachable this month without writing a
          line of code.
        </p>

        <h2>Is GrokBot good enough for most people?</h2>
        <p>
          Right now, for most people, it is the best entry point available,
          and I do not say that as a Grok partisan. The place almost everyone
          dies is the setup tax, and GrokBot removes most of it. You describe
          the job in plain language, point it at the accounts it needs, and it
          runs. You can hand it a routine and walk away from the keyboard,
          which was not true of consumer AI a year ago.
        </p>
        <p>
          Our chief of staff agent lives there. It routes work, keeps
          specialists on call, logs into the browser-shaped parts of the
          business a terminal cannot touch, and follows through while I am
          somewhere else. It is not the most powerful thing we run. It asked
          the least of me to start.
        </p>
        ...
        <p>
          ...
          The terminal. The deploys. The long work of reading forty documents
          to find the one number that contradicts the other thirty-nine. Memory
          on disk as files I can open and correct.
        </p>
        <p>
          That is the difference between an agent that reports a problem and an
          agent that closes it, which we argued about in April:{" "}
          <a href="/blog/claude-code-vs-cowork">
        ...
        <p>
          The cost is real. You are managing files, and when something breaks
          you have to read what broke. It is a steeper start than GrokBot and I
          would not send someone there first.
        </p>

        <h2>Why run both instead of picking one?</h2>
        <p>
          They cover different halves, and the handoff between them turned out
          to be the part that matters. Neither would be better if I forced it
          onto the other platform, and that is the claim here I am most
          confident about, because I have tried it.
        </p>
        <p>
          The loop is spec, build, verify, approve. A brief gets drafted in one
          place and pressure-tested in the other. When one model drifts, or
          reads a design problem as a factual one, the other catches it, and I
          would rather find that on my own screen than in a client&apos;s
          inbox.
        </p>
        <p>
          That is redundancy, not busywork. The distinction is whether the
          second pass can actually disagree with the first. Two agents on one
          platform share the same blind spots, so they mostly agree, which
          feels like verification and is not.
        </p>
        <p>
          It is not free either. You feed two memories instead of one, and you
          have to decide which half owns a job, or the work gets done twice, or
          dropped because each side assumed the other had it. If you will not
          maintain both, run one loop on the easier platform and stay there
        ...
        <p>
          ...
          in front of a client. It went in one correction at a time, usually
          right after something went wrong. The roster version is here:{" "}
          <a href="/blog/build-your-own-ai-team">one person, five agents</a>.
        </p>

        <h2>Where do you start?</h2>
        <p>
          Start on the easier platform, with one job you keep forgetting to do.
          Not the most important job. The one you resent, because you will need
          the motivation in week three when the first version is worse than
          doing it yourself.
        </p>
        <p>
          Give it the five pieces. Name the job, write down the context and
          keep writing it every time it gets something wrong, connect it to the
          accounts it needs, put it on a schedule, decide where the output
          lands. Then leave it alone for a week.
        </p>
        <p>
          You move it down to the harder layer when you hit the wall, and the
          wall is specific: it needs to change something in a system it cannot
          reach. Not before. Most people shop for the powerful tool first and
          never get an agent running at all.
        </p>
        <p>
          The roster we published in July has already moved one agent onto a
          different platform, and nothing underneath it changed. That is why I
          am not nervous about whatever ships next spring.
        </p>
        <p>
          The end goal is not a bigger roster. It is less of your week spent on
          the repetitive grind and more of it spent on judgment, on high-level
          thinking, on the decisions only a person should be making. Agents
          take the loop work. You keep the taste and the call.
        </p>
